...
export default defineEventHandler(async (event) => {
    ...
    const blueprintId = getRouterParam(event, 'blueprintId')!
    const db = useDB()
    ...
    if (fulfillmentProviderName !== 'printify' || !blueprintId) {
        return { variants: [] }
    }

    const apiKey = (settings?.printifyApiKey ?? config.printifyApiKey) as string
    if (!apiKey) {
        throw createError({ statusCode: 400, statusMessage: 'No Printify API key configured. Please configure it in Settings.' })
    }
    ...
    try {
        ...
        throw createError({ statusCode: 500, statusMessage: err.message ?? 'Failed to load variants' })
    }
})
